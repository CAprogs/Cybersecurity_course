...
class MyScanner:
    ...
    def perform_scan(self):
        ...
        self.scan_result = self.nm.scan(hosts=self.?, arguments='-?')
        ...
        for host in self.nm.all_?():
            try:
                # Attempt to access detailed information for each discovered host
                detailed_info = self.scan_result["scan"][?]
                # Print the host's IP, state (up or down)
                print(f"Host: {?} is {detailed_info['status']['state']}")
            except KeyError:
